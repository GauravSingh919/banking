...
if (isset($_SESSION['auth_active']) && $_SESSION['auth_active'] == 1) {
    ...
    if (isset($_POST['withdraw'])) {
        $withdraw_amount = isset($_POST['withdraw_amount']) ? intval($_POST['withdraw_amount']) : 0;
        if ($withdraw_amount > $user_balance) {
            echo '<script type="text/javascript">alert("Withdrawal amount cannot be more than balance.")</script>';
        } else if ($withdraw_amount <= 0) {
            echo '<script type="text/javascript">alert("Invalid Withdrawal amount.")</script>';
        } else {
            $user_account_no = $user_account_query_data_array['account_no'];
            $transaction_query = "INSERT INTO `transactions` (`user_id`, `type`, `amount`) VALUES ($user_id, 'debit', $withdraw_amount)";
            ...
            if ($transaction_query_request) {
                ...
                echo '<script type="text/javascript">alert("Withdrawal successful.")</script>';
            } else {
                        ...
                        <div class="container">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" step="any" min="1" class="form-control col-sm-6 text-right"
                                    name="withdraw_amount" value="1" required>
                            </div>
                        </div>
                    </div>
                    <div class="card-footer">
                        <div class="d-flex w-100">
                            <button type="submit" name="withdraw" class="btn btn-primary mr-2">Submit</button>
                            <a href="transactions.php?customer_id=<?= $user_id ?>" class="btn btn-default">Cancel</a>
                        </div>
                    </div>
                </form>
            </div>
